...
if data is not None:
    ...
    # --- HEADER ---
    st.title(f"📊 QuantEdge Intelligence: {ticker_input}")
    ...
    with col2:
        ...
        if current_r2 > 0.7: regime = "🚀 Strong Trend"
        elif current_r2 < 0.3: regime = "📉 Mean Reverting / Choppy"
        else: regime = "⚖️ Transitional"
        ...
        if abs(z_score) > 2:
            st.error(f"⚠️ ANOMALY DETECTED: {z_score:.2f}σ Move")
        else:
            st.success("Normal Price Distribution")

    # --- MODULE 2: POSITION SIZING ENGINE ---
    st.divider()
    st.subheader("⚡ Smart Position Sizing (Kelly & ATR)")
    ...
    c2.metric("ATR Based Stop (2x)", f"₹{atr*2:.2f}")
    c3.metric("Recommended Units", f"{units} Shares", delta=f"₹{units*data['Close'].iloc[-1]:,.0f} Exposure")

    # --- MODULE 3: PROBABILITY FORECASTER ---
    st.divider()
    st.subheader("🔮 Mean Reversion & Breakout Probabilities")
    ...
    st.divider()
    st.subheader("🧪 Signal Quality Backtest (SMA Crossover)")
    ...
    fig_bt.update_layout(template="plotly_dark", title="Cumulative Growth (₹1 Base)")
